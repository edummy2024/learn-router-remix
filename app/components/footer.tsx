...
export default function Footer() {
  ...
  return (
    <footer className="min-h-[154px] bg-white grid place-content-center">
      <div className="flex justify-center flex-col py-6 gap-2 max-w-[1100px] px-10">
        <ul className="flex lg:gap-2 gap-1.5 text-xs font-normal text-gray-500 flex-wrap">
          {
            languages.slice(0, 8).map((lang) => (
              <li key={lang.code}>
                <a href="" className="hover:underline">{lang.name}</a>
              </li>
            ))
          }
          <li>
            <button
              ...
              className="border border-gray-400 p-0.5 bg-gray-100 rounded-sm hover:bg-gray-200">
              <LucidePlus size={16} /></button>
          </li>
        </ul>
        <hr  />
        <ul className="flex lg:gap-2 gap-1.5 text-xs font-normal text-gray-500 flex-wrap">
        ...
        <div>
          <span className="text-xs font-normal text-gray-500">Meta &copy; 2024</span>
        </div>
      </div>
    </footer>
  )
